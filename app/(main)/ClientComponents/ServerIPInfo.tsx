...
import type { IPInfo } from "@/lib/ip-info-service";
...
export default function ServerIPInfo({ server_id }: { server_id: number }) {
  ...
  const { data, error } = useSWRImmutable<IPInfo>(
  ...
  if (!data) {
    return (
      <div className="mb-4 flex min-h-[42px] items-center gap-2 text-xs font-medium opacity-70">
        <Loader visible={true} />
        <span className={cn("leading-none")}>{t("analysing")}</span>
      </div>
    );
  }
